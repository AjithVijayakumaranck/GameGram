...
const EditProf = ({ userDetails,setFile,file}) => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://gamegram.ga/api/getuserprofile/${userId}`).then((response) => {
      ...
      setFile({...file,
        file:`http://gamegram.ga/api/images/postimages/${response.data.response.profilePic}`,
        fileUrl:`http://gamegram.ga/api/images/postimages/${response.data.response.profilePic}`})
    }).catch((err) => {
      console.log(err);
    })
  }, [])
  ...
  const saveChangesHandler = () => { 
    ...
    axios.post(`http://gamegram.ga/api/updateuserprofile`,data,{
