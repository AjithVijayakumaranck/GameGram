...
const Notification = () => {
  ...
  useEffect(() => {
  axios.get(`http://localhost:5000/getNotification/${localStorage.getItem('user')}`).then((response=>{
  ...
  return (
    ...
    <p className='pl-3 cursor-pointer text-main' onClick={()=>{
         ...
         navigate(`/userprofile/${result.userId }`)
          ...
          <img src={`http://localhost:5000/images/postimages/${result.post}`} className='w-12 mx-2 rounded-sm' alt="" />
        </div>
       )
      })
     }
